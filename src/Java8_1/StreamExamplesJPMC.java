...
public class StreamExamplesJPMC {
    ...
    public static void main(String[] args) {
        ...
        Stream<String> distinctSubjects =
                students.stream().flatMap(student -> student.getSubjects().stream()).distinct();
        distinctSubjects.forEach(System.out::println);

        // student sorted by marks

        List<Student> sortedStudent =
                students.stream().sorted((s1, s2) -> Double.compare(s2.getMarks(), s1.getMarks())).collect(Collectors.toList());
        ...
        List<Student> object = students.stream().sorted((s1, s2) -> Double.compare(s2.getMarks(), s1.getMarks())).limit(3).collect(Collectors.toList());
        object.stream().forEach(System.out::println);

        System.out.println("Skipping top 2 students by marks: ");
        List<Student> resultSkip2 = students.stream().sorted((s1, s2) -> Double.compare(s2.getMarks(), s1.getMarks())).skip(2).collect(Collectors.toList());
        resultSkip2.stream().forEach(System.out::println);
